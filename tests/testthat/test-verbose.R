...
test_that("verbose parameter controls diagnostic output", {
  ...
  output_quiet <- capture.output({
    result_quiet <- runPlugin(test_plugin, wave, verbose = FALSE)
  }, type = "message")
  
  # Capture output with verbose=TRUE
  output_verbose <- capture.output({
    result_verbose <- runPlugin(test_plugin, wave, verbose = TRUE)
  }, type = "message")
...
test_that("verbose works with custom block/step sizes", {
  ...
  output_verbose <- capture.output({
    result <- runPlugin(test_plugin, wave, blockSize = 2048, stepSize = 512, verbose = TRUE)
  }, type = "message")
  ...
  output_quiet <- capture.output({
    result_quiet <- runPlugin(test_plugin, wave, blockSize = 2048, stepSize = 512, verbose = FALSE)
  }, type = "message")
...
test_that("verbose default is FALSE", {
  ...
  output_default <- capture.output({
    result_default <- runPlugin(test_plugin, wave)
  }, type = "message")
  ...
  output_explicit <- capture.output({
    result_explicit <- runPlugin(test_plugin, wave, verbose = FALSE)
  }, type = "message")
